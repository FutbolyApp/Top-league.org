...
async function testAdminRequests() {
  try {
    console.log('🔍 Testing admin requests and notifications...');
    ...
    // Check users
    const usersResult = await db.query('SELECT id, nome, cognome, email, ruolo FROM users ORDER BY id');
    ...
    // Check leghe
    const legheResult = await db.query('SELECT id, nome, admin_id FROM leghe ORDER BY id');
    ...
    console.error('❌ Error testing admin requests:', error);
  }
}
